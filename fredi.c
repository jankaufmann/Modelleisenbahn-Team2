...
while (1)
  {
	  ...
	  processTimerActions();
	  processADC();
	  //if(slotArray[0].slot != 0) {sendLocoNet4BytePacketTry(0xA0,slotArray[0].slot,(byte)test,0x01);_delay_ms(500);}
	  //if(slotArray[0].slot != 0) {sendLocoNetWriteSlotData(&slotArray[0]);_delay_ms(500);}
	  if(test>126){
		   test = 0;
	  }
  } // end of while(1)
